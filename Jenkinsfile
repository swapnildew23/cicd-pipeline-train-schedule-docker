        ...
        stage('Deploy to Production Server') {
            ...
            steps {
                ...
                withCredentials([usernamePassword(credentialsId: 'web-server-login', usernameVariable: 'USERNAME', passwordVariable: 'PASSWORD')]) {
                    script {
                        sh "sshpass -p '$PASSWORD' -v ssh -o StrictHostKeyChecking=no $USERNAME@$prod_ip \"sudo docker pull swap/test-train-schedule:${env.BUILD_NUMBER}\" "
                        try {
                            sh "sshpass -p '$PASSWORD' -v ssh -o StrictHostKeyChecking=no $USERNAME@$prod_ip \"sudo docker stop test-train-schedule\" "
                            sh "sshpass -p '$PASSWORD' -v ssh -o StrictHostKeyChecking=no $USERNAME@$prod_ip \"sudo docker rm test-train-schedule\" "
                        } catch (err) {
                            echo: 'caught error: $err'
                        }
